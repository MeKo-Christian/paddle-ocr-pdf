...
def process_pdf(input_pdf_path, output_pdf_path):
    ...
    for page_number in tqdm.tqdm(
        ...
        if not image_list:
            continue
        dpi = int(
            max(max(i[2], i[3]) for i in page.get_images())
            / min(page.rect.width / 72, page.rect.height / 72)
        )
        pix = page.get_pixmap(dpi=dpi)
        cim = np.frombuffer(pix.samples, dtype=np.uint8).reshape(pix.h, pix.w, pix.n)
        cim = np.ascontiguousarray(cim[..., [2, 1, 0]])
        # No need to rotate cim, as pix is already rotated according to page.rotation
        if args.cv:
